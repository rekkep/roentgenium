...
class RoentgeniumGui(QWidget):
    def __init__(self, CONFIG, groups):
        ...
        entries = self.get_entries(selected_group)

        for input_field in self.input_fields:
            self.setup_text_input(input_field)  # Input field
        self.setup_labels()  # Lable list for entries

        # Show the parent widget
        self.central_widget.show()

    def get_entries(self, selected_group):
        ENTRIES = [self.groups[group].entries for group in self.groups]
        self.input_filed = self.groups[selected_group].input_fields
        return ENTRIES
    ...
    def setup_labels(self):
        ...
        for _ in range(self.CONFIG.ENTRIES_VISIBLE_ENTRIES):
            ...
            self.label.setProperty("selected", False)
